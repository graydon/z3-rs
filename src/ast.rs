...
impl<'ctx> Ast<'ctx> {
    ...
    pub fn pb_le(&self, other: &[&Ast<'ctx>], coeffs: Vec<i32>, k: i32) -> Ast<'ctx> {
            Ast::new(self.ctx, unsafe {
                ...
                Z3_mk_pble(self.ctx.z3_ctx, tmp.len() as u32, tmp.as_ptr(), tmp_coeffs.as_mut_ptr(), k)
            })
    }
    pub fn pb_ge(&self, other: &[&Ast<'ctx>], coeffs: Vec<i32>, k: i32) -> Ast<'ctx> {
            Ast::new(self.ctx, unsafe {
                ...
                Z3_mk_pbge(self.ctx.z3_ctx, tmp.len() as u32, tmp.as_ptr(), tmp_coeffs.as_mut_ptr(), k)
            })
    }
    pub fn pb_eq(&self, other: &[&Ast<'ctx>], coeffs: Vec<i32>, k: i32) -> Ast<'ctx> {
            Ast::new(self.ctx, unsafe {
                ...
                Z3_mk_pbeq(self.ctx.z3_ctx, tmp.len() as u32, tmp.as_ptr(), tmp_coeffs.as_mut_ptr(), k)
            })
    }
}
